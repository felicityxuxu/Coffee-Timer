...
struct ContentView: View {
        ...
        ZStack {
            ...
            LinearGradient(
                gradient: Gradient(colors: [
                    Color(uiColor: UIColor(red: 0.98, green: 0.95, blue: 0.93, alpha: 1.0)),
                    Color(uiColor: UIColor(red: 0.95, green: 0.92, blue: 0.90, alpha: 1.0))
                ]),
                startPoint: .top,
                endPoint: .bottom
            )
            ...
            VStack(spacing: 4) {
                ...
                Text("Welcome to Our Virtual Coffee Shop!")
                    ...
                    .shadow(color: .brown.opacity(0.2), radius: 1, x: 0, y: 1)
                
                LottieView(animationName: "coffee-animation", loopMode: .loop, scale: 1.2)
                    .frame(width: 500, height: 380)
                
                Text("Focus to collect unique coffee stickers! ✨")
                    .font(.custom("Chalkboard SE", size: 18))
                    .foregroundColor(Color.brown.opacity(0.7))
                    .shadow(color: .brown.opacity(0.1), radius: 1, x: 0, y: 1)
                
                // Button group with improved visual hierarchy
                VStack(spacing: 16) {
                    ForEach(["Start Focus Time", "Coffee Collections", "Background Music"], id: \.self) { buttonText in
                        Button(action: {
                            switch buttonText {
                            case "Start Focus Time": showingTimer = true
                            case "Coffee Collections": showingCollection = true
                            case "Background Music": showingMusicSelection = true
                            default: break
                            }
                        }) {
                            HStack(spacing: 12) {
                                Image(systemName: buttonIcon(for: buttonText))
                                    .font(.system(size: 24))
                                Text(buttonText)
                                ...
                                RoundedRectangle(cornerRadius: 20)
                                    ...
                                    .background(
                                        RoundedRectangle(cornerRadius: 20)
                                            ...
                                            .shadow(color: .brown.opacity(0.1), radius: 5, x: 0, y: 2)
                                    )
                            )
                            .contentShape(Rectangle())
                        }
                        .buttonStyle(ScaleButtonStyle())  // Add subtle scale animation
                    }
                }
                .padding(.top, 10)
                .padding(.bottom, 30)
    ...
    private func buttonIcon(for text: String) -> String {
        switch text {
        case "Start Focus Time": return "play.circle.fill"
        case "Coffee Collections": return "star.circle.fill"
        case "Background Music": return "music.note"
        default: return ""
        }
    }
}

// Add subtle scale animation to buttons
